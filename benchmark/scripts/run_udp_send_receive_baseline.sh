...
usage() {
  cat <<'EOF'
Usage: benchmark/scripts/run_udp_send_receive_baseline.sh [options]
...
IOCORO_BIN="$BUILD_DIR/benchmark/iocoro_udp_send_receive"
ASIO_BIN="$BUILD_DIR/benchmark/asio_udp_send_receive"
...
echo "Running udp send/receive benchmark"
...
for pair in "${scenario_pairs[@]}"; do
  ...
  for ((i = 0; i < ITERATIONS; ++i)); do
    ...
    pps="$(bench_extract_metric "pps" "$line")"
    if [[ -z "$pps" ]]; then
      echo "Failed to parse iocoro pps: $line" >&2
      exit 1
    fi
    iocoro_runs+=("$pps")
  done
  ...
  for ((i = 0; i < ITERATIONS; ++i)); do
    ...
    pps="$(bench_extract_metric "pps" "$line")"
    if [[ -z "$pps" ]]; then
      echo "Failed to parse asio pps: $line" >&2
      exit 1
    fi
    asio_runs+=("$pps")
  done
  ...
  scenario_json+=("{\"sessions\":$sessions,\"msgs\":$msgs,\"msg_bytes\":$msg_bytes,\"iocoro_pps_runs\":[${iocoro_runs_json}],\"asio_pps_runs\":[${asio_runs_json}],\"iocoro_pps_median\":$iocoro_median,\"asio_pps_median\":$asio_median,\"ratio_vs_asio\":$ratio,\"min_ratio\":$min_ratio_json,\"pass\":$pass_bool}")

  echo "  iocoro median pps: $iocoro_median"
  echo "  asio   median pps: $asio_median"
...
printf '| %-8s | %-6s | %-9s | %-16s | %-14s | %-14s | %-10s | %-6s |\n' \
  "sessions" "msgs" "msg_bytes" "iocoro_pps_median" "asio_pps_median" "ratio_vs_asio" "min_ratio" "gate"
printf '|-%-8s-|-%-6s-|-%-9s-|-%-16s-|-%-14s-|-%-14s-|-%-10s-|-%-6s-|\n' \
  "--------" "------" "---------" "----------------" "--------------" "--------------" "----------" "------"
