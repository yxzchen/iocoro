...
int main(int argc, char* argv[]) {
  ...
  if (workers <= 0) {
    std::cerr << "iocoro_thread_pool_scaling: workers must be > 0\n";
    return 1;
  }
  if (tasks == 0) {
    std::cerr << "iocoro_thread_pool_scaling: tasks must be > 0\n";
    return 1;
  }

  iocoro::thread_pool pool{static_cast<std::size_t>(workers)};
  ...
  std::cout << "iocoro_thread_pool_scaling"
