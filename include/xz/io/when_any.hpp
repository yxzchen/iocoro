...
#include <xz/io/detail/when/when_any_state.hpp>
...
auto when_any(std::vector<awaitable<T>> tasks) -> awaitable<std::pair<
  ...
  auto st = std::make_shared<::xz::io::detail::when_any_container_state<T>>(ex);
  ...
  if constexpr (std::is_void_v<T>) {
    ...
    std::scoped_lock lk{st->result_m};
    XZ_ENSURE(st->result.has_value(), "when_any(vector): missing value");
