...
class tcp_socket : public ::iocoro::basic_socket<::iocoro::detail::ip::tcp_socket_impl> {
 public:
  using base_type = ::iocoro::basic_socket<::iocoro::detail::ip::tcp_socket_impl>;

  tcp_socket() noexcept = default;

  explicit tcp_socket(::iocoro::executor ex);
  explicit tcp_socket(::iocoro::io_context& ctx);

  auto async_connect(::iocoro::use_awaitable_t, endpoint const& ep)
    -> ::iocoro::awaitable<std::error_code>;

  auto async_read_some(::iocoro::use_awaitable_t, void* data, std::size_t size)
    -> ::iocoro::awaitable<::iocoro::expected<std::size_t, std::error_code>>;

  auto async_write_some(::iocoro::use_awaitable_t, void const* data, std::size_t size)
    -> ::iocoro::awaitable<::iocoro::expected<std::size_t, std::error_code>>;
};
